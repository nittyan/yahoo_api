...
class Condition(object):
	...
	def to_parameter(self):
		...
		if not self.appid or not self.sentence or not self.results:
			raise ParameterAbsenceError('appid or sentence or result is not set')
...
class MorphologicalAnalysis(object):
	...
	def parse_ma(self, soup):
		...
		words = [
			Word(
				surface = word.surface.get_text() if word.find('surface') else '', 
				reading = word.reading.get_text() if word.find('reading') else '', 
				pos = word.pos.get_text() if word.find('pos') else '', 
				baseform = word.baseform.get_text() if word.find('baseform') else '', 
...
class Word(object):
	def __init__(self, count=0, surface='', reading='', pos='', baseform='', feature=''):
		self.count = count
